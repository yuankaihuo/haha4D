...
namespace itk {
		...
		::BeforeThreadedGenerateData()
	{
		...
		//of atlas modalities
		if (this->m_timePoints == 1 && this->m_TargetImage.size() != 1 && this->m_TargetImage.size() != this->m_NumberOfAtlasModalities)
		{
		...
		::ThreadedGenerateDataForWeightedAveraging(const RegionType & region, ThreadIdType threadId)
	{
		...
		SizeValueType numberOfTargetModalities;
		if (this->m_timePoints == 1) {
			numberOfTargetModalities = this->m_TargetImage.size();
		}
		else {
		...
		for (ItN.GoToBegin(); !ItN.IsAtEnd(); ++ItN)
		{
			...
			//printf("count=%d,  [x=%d, y=%d, z=%d]\n", count++, currentCenterIndex.m_Index[0], currentCenterIndex.m_Index[1], currentCenterIndex.m_Index[2]); //yh add
