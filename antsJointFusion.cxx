...
int antsJointFusion( itk::ants::CommandLineParser *parser )
{
  ...
  typename OptionType::Pointer timePointOption = parser->GetOption("time-points");
  unsigned int timePoints = 1;
  if (timePointOption && timePointOption->GetNumberOfFunctions() > 0)
  {
	  timePoints = parser->Convert<unsigned int>(timePointOption->GetFunction(0)->GetName());
  }
  fusionFilter->SettimePoints(timePoints);
  ...
  numberOfTargets = targetImageOption->GetNumberOfFunctions();
  if( targetImageOption && targetImageOption->GetNumberOfFunctions() )
    ...
    if( targetImageOption->GetFunction( 0 )->GetNumberOfParameters() == 0 )
	{
		for (unsigned int m = 0; m < numberOfTargets; m++)
		{
			...
			if (targetImageOption->GetFunction(m)->GetNumberOfParameters() == 0)
			{
				typename ImageType::Pointer targetImage = ITK_NULLPTR;
				std::string targetFile = targetImageOption->GetFunction(m)->GetName();
...
void InitializeCommandLineOptions( itk::ants::CommandLineParser *parser )
{
  ...
  {
	  std::string description =
		  std::string("How many time points in 4D MAS");
